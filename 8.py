my_string = "Prevailed sincerity behaviour to so do principle mr. As departure at no propriety zealously my." \
            " On dear rent if    girl view. First on smart there he sense. Earnestly enjoyment her you resources." \
            " Brother chamber ten old against. Mr be cottage so    related minuter is." \
            " Delicate say and blessing ladyship exertion few margaret." \
            " Delight herself welcome against smiling its for. " \
            "Suspected    discovery by he affection household of principle perfectly he."

print(my_string.count('a'))

